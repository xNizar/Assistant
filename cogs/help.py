...
class Help(formatter.HelpFormatter):
    ...
    async def send(self, dest, content=None, embeds=None):
        ...
        page_msg = await dest.send(
            self.bot.bot_prefix + "There are {} help pages. Send a number to see the corresponding page. Send any other message to exit.".format(
                len(embeds)))
    ...
    async def format(self, ctx, command):
        ...
        if isinstance(command, discord.ext.commands.core.Command):
            ...
            emb['embed']['description'] = '❯Usage `{0}`'.format(self.get_command_signature())
        ...
        def category(tup):
            ...
            cog = tup[1].cog_name
            return '{0}'.format(cog) if cog is not None else 'No Category'
    ...
    async def format_help_for(self, ctx, command_or_bot, reason: str = None):
        ...
        for field in emb['fields']:
            ...
            if len(txt) > 1000:
                ...
                del embed
                embed = discord.Embed(color=self.color, **emb['embed'])
                embed.set_author(name='{} Help Manual Page {}'.format(self.bot.user.name, len(embeds) + 1),
                                 icon_url=self.avatar)
                embed.set_footer(**emb['footer'])
