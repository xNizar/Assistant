...
def update_bot(message):
    ...
    if ('up to date' in update or 'fast-forward' in update) and message:
        ...
        for i in range(int(version)):
            ...
            field = g.execute(["git", "log", "--pretty=oneline", "--abbrev-commit", "--shortstat", commits[i],
                               "^{0}".format(commits[i + 1])])
            field = field[8:].strip()
    ...
    async with session.get(url, params=params, headers=headers) as resp:
        if resp.status != 200:
            ...
            async with session.get(
                    "https://www.googleapis.com/customsearch/v1?q=" + quote_plus(query) + "&start=" + '1' + "&key=" +
                    config['google_api_key'] + "&cx=" + config['custom_search_engine']) as resp:
